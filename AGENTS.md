...
This file is the working guide for agents editing `cloudflare-repos`.

## What this repo is

`cloudflare-repos` is a single-page static catalog of public repositories from
Cloudflare's `cloudflare` GitHub organization. It follows a
...
- `cloudflare/worker.js`: proxy used for
  `https://glaubi.net/cloudflare`.

## Catalog rules

Included repositories must be:

- Owned by the `cloudflare` GitHub org.
...
1. Workers, Pages & Runtime
2. AI, Agents & MCP
3. Networking, Protocols & Performance
4. Security, PKI & Cryptography
5. SDKs, APIs & Infrastructure as Code
6. Observability, Systems & Libraries
7. Docs, Templates & Community
...
That command queries GitHub for current `cloudflare` repositories
